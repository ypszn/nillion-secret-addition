...
// ../client-vms/src/nilvm/client.ts
...
var Log4 = debug4("nillion:react-hooks");

// src/nillion-provider.tsx
var NillionContext = createContext(
  void 0
);
var client = NillionClient3.create();
var NillionProvider = (props) => {
  const existingQueryClient = useContext(QueryClientContext);
  const [queryClient] = useState(new QueryClient());
  const [nillionClient] = useState(client);
  const { children, network, config } = props;
  useEffect(() => {
    if (existingQueryClient) Log4("Reusing detected react query context.");
    else Log4("No react query context detected; creating one.");
    ...
    nillionClient.setNetworkConfig(combined);
  }, []);
  const context = {
  ...
  if (existingQueryClient) {
    return /* @__PURE__ */ React.createElement(NillionContext.Provider, { value: context }, children);
  }
  return /* @__PURE__ */ React.createElement(QueryClientProvider, { client: queryClient }, /* @__PURE__ */ React.createElement(NillionContext.Provider, { value: context }, children));
};

// src/use-nil-compute.ts
import { useMutation } from "@tanstack/react-query";

// src/nil-hook-base.ts
var NilHookState = {
  Idle: {
    status: "idle",
    isIdle: true,
...
function useNillion() {
  const context = useContext2(NillionContext);
  if (!context) {
    throw new Error(
...
var useNilCompute = () => {
  ...
  const mutationFn = async (args) => {
    const computeArgs = {
      bindings: args.bindings,
      values: args.values ?? NadaValues2.create(),
  ...
  return {
    ...
    executeAsync: async (args) => mutate.mutateAsync(args),
    ...nilHookBaseResult(mutate)
  };
};

// src/use-nil-compute-output.ts
import { useMutation as useMutation2 } from "@tanstack/react-query";
var useNilComputeOutput = () => {
  const { client: nilClient } = useNillion();
  const mutationFn = async (args) => {
...
// src/use-nil-delete-value.ts
import { useMutation as useMutation3, useQueryClient } from "@tanstack/react-query";

// src/cache-key.ts
var REACT_QUERY_STATE_KEY_PREFIX = "__NILLION";
var createStoreCacheKey = (id) => [REACT_QUERY_STATE_KEY_PREFIX, "STORE", id];

// src/use-nil-delete-value.ts
var useNilDeleteValue = () => {
  const { client: nilClient } = useNillion();
...
// src/use-nil-fetch-store-acl.ts
import { useMutation as useMutation4 } from "@tanstack/react-query";
var useNilFetchStoreAcl = () => {
  const { client: client2 } = useNillion();
  const mutationFn = async (args) => {
    const response = await client2.fetchStoreAcl(args);
    if (response.err) throw response.err;
    return response.ok;
  };
  const mutate = useMutation4({
    mutationFn
  });
  return {
    execute: (args) => {
  ...
  const mutationFn = async (args) => {
    ...
    if (options.staleAfter) {
      ...
      if (cachedData) {
        ...
        const staleTimeThreshold = options.staleAfter ?? 5 * 60 * 1e3;
        if (dataAge <= staleTimeThreshold && cachedData.status === "success" && cachedData.data) {
          Log4("Cache hit: %O", cachedData.data);
...
// src/use-nil-set-store-acl.ts
import { useMutation as useMutation6 } from "@tanstack/react-query";
var useNilSetStoreAcl = () => {
  const { client: client2 } = useNillion();
  const mutationFn = async (args) => {
    const response = await client2.setStoreAcl(args);
    if (response.err) throw response.err;
    return response.ok;
  };
  const mutate = useMutation6({
    mutationFn
  });
  return {
    execute: (args) => {
...
// src/use-nil-store-program.ts
import { useMutation as useMutation7 } from "@tanstack/react-query";
var useNilStoreProgram = () => {
  const { client: nilClient } = useNillion();
  const mutationFn = async (args) => {
...
// src/use-nil-update-value.ts
import { useMutation as useMutation10, useQueryClient as useQueryClient5 } from "@tanstack/react-query";
var useNilUpdateValue = () => {
  const { client: nilClient } = useNillion();
...
// src/use-nillion-auth.ts
import { useContext as useContext3 } from "react";
function useNillionAuth() {
  const context = useContext3(NillionContext);
  if (!context) {
    throw new Error(
